...
//   List<MassageSchema>? massages;
  ...
  Future<MassageSchema?> sendImageMessage(
  ...
  Future<MassageSchema?> sendMassage({
    ...
    try {
      ...
      //   var usersChatedM = sharedPreferences.get("usersChatedM");
      var box =  await Hive.openBox('data');

      var usersChatedM = box.get('usersChatedM');

      print(intl.DateFormat('dd/MM/yyyy').format(
      ...
      if (toUserId != null && toUserId != "") {
        if (!usersChatedM[intl.DateFormat('dd/MM/yyyy').format(
                DateTime.fromMillisecondsSinceEpoch(
                    DateTime.now().millisecondsSinceEpoch))]
            ?.contains(toUserId)) {
          print({
            intl.DateFormat('dd/MM/yyyy').format(
                DateTime.fromMillisecondsSinceEpoch(
                    DateTime.now().millisecondsSinceEpoch)): [
              ...usersChatedM[intl.DateFormat('dd/MM/yyyy').format(
                  DateTime.fromMillisecondsSinceEpoch(
                      DateTime.now().millisecondsSinceEpoch))],
              toUserId
            ]
          });
          box.put("usersChatedM", {
            intl.DateFormat('dd/MM/yyyy').format(
                DateTime.fromMillisecondsSinceEpoch(
                    DateTime.now().millisecondsSinceEpoch)): [
              ...usersChatedM[intl.DateFormat('dd/MM/yyyy').format(
          ...
          print("STAGE 3: send notification");
        }
